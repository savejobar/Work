...
- **TSB (Teunter-Syntetos-Babai)** — если доля нулевых месяцев превышает порог (по умолчанию 40%). Оптимизирован для нерегулярного спроса, коэффициенты α и β подбираются автоматически.
...
Минимальная доля нулевых месяцев, при которой применяется TSB. Например, если запчасть продавалась лишь в 5 из 12 месяцев (≈58% нулей) — система выберет TSB. Порог настраивается в параметрах.
