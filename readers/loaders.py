...
def preprocess_repair_parts(file_path: str) -> pd.DataFrame:
    ...
    # Выделяем подъёмники
    df["Машина_подъемник"] = df["Машина"].str.contains(r"подъ[её]мник", case=False, na=False, regex=True)

    if not df["Машина_подъемник"].any():
        raise ValueError(
            "В отчете 'Запчасти списанные в ремонт' не найдено строк по подъемной технике."
        )
