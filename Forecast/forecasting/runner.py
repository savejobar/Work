...
======================
Оркестрация прогнозирования: поиск запчасти, построение серии,
запуск модели, вывод графика и таблицы результатов.

Важно: df должен быть результатом run_full_pipeline() с уже применённой
fill_missing_months() — тогда каждая группа начинается с первой своей
активности, и никакого хардкода дат не нужно.
"""
...
def _get_series(
    ...
    а не первый месяц активности группы вообще.
    Это важно: Продажа и Ремонт могут стартовать в разное время.
    """
...
def find_groups_by_article(df: pd.DataFrame, article: str) -> list[dict]:
    ...
    if not hits and "Список аналогов" in df.columns:
        for _, row in df.drop_duplicates("Номер группы").iterrows():
            analogs = row.get("Список аналогов")
...
def run_group_forecast(
    df: pd.DataFrame,
    group_id: int,           # ← теперь принимаем id напрямую
    steps: int = 3,
    ...
    nomenclature = str(meta_row.get("Номенклатура", "—"))[:80]
    article_out  = str(meta_row.get("Артикул", "—"))
...
def _draw_panel(
    ...
    fc_y = [float(raw.values[-1])] + list(fc.values)
